...
<xsl:output method = "xml" standalone ="yes" encoding="UTF-8"/> 
	<!--NMEA VALIDATE To TAIP LN.xsl (c) SunGard HTE Inc-->
		
	<!--NMEA VALIDATE TO TAIP LN STYLESHEET-->
							...
							<xsl:choose>
								...
								<xsl:when test = "$fromSystem = 'DDM'">
									<xsl:choose>
										...
										<xsl:when test = "$toSystem = 'DD'">
											<xsl:value-of select="substring(concat($latD,substring-after((number(concat($latM,'.',$latm))div 60),'.'),'000000000'),1,9)" />
										</xsl:when>
								...
								<!--ALREADY IN FORMAT REQUIRED-->
									<xsl:choose>
										<xsl:when test = "$toSystem = 'DD'">
											<xsl:value-of select="substring(concat(substring-before(concat($lat,'.'),'.'),substring-after(concat($lat,'.'),'.'),'000000000'),1,9)" />
										</xsl:when>
									</xsl:choose>
								</xsl:otherwise>
							</xsl:choose>
...
<!--CONVERT LONGITUDE FOR GLL-->
							<xsl:choose>
								...
								<xsl:when test = "$fromSystem = 'DDM'">
									<xsl:choose>
										<xsl:when test = "$toSystem = 'DD'">
											<xsl:value-of select="substring(concat($longD,substring-after((number(concat($longM,'.',$longm))div 60),'.'),'0000000000'),1,10)" /> 
										</xsl:when>
									</xsl:choose>
								</xsl:when>
								<xsl:otherwise>
								<!--ALREADY IN FORMAT REQUIRED-->
									<xsl:choose>
										<xsl:when test = "$toSystem = 'DD'">
											<xsl:value-of select="substring(concat(substring-before(concat($long,'.'),'.'),substring-after(concat($long,'.'),'.'),'000000000'),1,10)" />
										</xsl:when>
									</xsl:choose>
								</xsl:otherwise>
							</xsl:choose>
<!--END CONVERT LONGITUDE FOR GLL-->	
					...
					<xsl:otherwise>
						...
						<rawMessage>NO VALID SATELLITE DATA</rawMessage>
						<Type>0</Type>
					</xsl:otherwise>
					</xsl:choose>
		</xsl:when>
		...
		<xsl:when test = "$transaction ='GPGGA' or $transaction ='$GPGGA'">
			...
			<!--0 - processed, 1 - processed warn, 2 - processed error, 3 - error -->
					...
					<xsl:when test ="string-length($latD)>0 and number($qual) > 0">
                                        <!--qual 0 is invalid, 1 is valid, 2 is Valid DGPS, 3 is Valid PPS -->
                                        <!-- sats 3 or more is needed to triangulate-->
						<MessageStatus>0</MessageStatus>
						<rawMessage>
							<xsl:value-of select ="string('RLN')"/>
							<!--since a fixed format, make sure spacing is in accordance with specification !-->
		...
		<xsl:when test = "$transaction ='GPRMC' or $transaction ='$GPRMC'">
			...
			<!--0 - processed, 1 - processed warn, 2 - processed error, 3 - error -->
					...
					<xsl:when test ="string-length($latD)>0 and $status='A'">
                                        <!--status A - is Active, V - is inactive -->
						<MessageStatus>0</MessageStatus>
						<rawMessage>
							<xsl:value-of select ="string('RLN')"/>
							<!--since a fixed format, make sure spacing is in accordance with specification !-->
