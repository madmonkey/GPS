...
<!--MapLite From TAIP DMS.xsl (c) SunGard HTE Inc-->
		
<!--MAPLITE FROM TAIP (DEGREES MINUTES SECONDS) STYLESHEET NON-STANDARD (SINCE CAD(s) ACCEPT ONLY DECIMAL DEGREES)-->
<!--USE THIS ONLY TO PLOT LOCALLY FROM A DMS TAIP DEVICE - THE CAD WILL NOT ACCEPT THIS FORMAT!!! -->

<xsl:output method = "xml" standalone ="yes"/> 
<xsl:template match="/">
	<GPSTransform>
	<xsl:variable name="transaction" select="substring(substring-after(.,'R'),1,2)"/>
	<!-- USES ID DEFINED IN HTE CONFIGURATION SET-UP !-->
	...
	<xsl:variable name="stat" select="'  '"/>
		...
		<xsl:when test = "$transaction ='PV'" >
			...
			<rawMessage>
				...
				<!--CONVERT FROM DEGREE MINUTE SECONDS TO DECIMAL DEGREES TO DECIMAL SECONDS!-->
