...
<xsl:output method = "xml" standalone ="yes" encoding="UTF-8"/> 
	<xsl:template match="/">
		...
		<xsl:variable name="toSeconds" select="'false'" />
		<!--Set toDecimal = true if the device transmits as Sexagesimal (DMS), the CAD, (CADV or CAD400) both expect Decimal (DD) coordinates --> 
		<GPSTransform>
		<xsl:choose>
					...
					<rawMessage>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:if test = "$toSeconds= 'true'">
									<xsl:value-of select="substring(concat($latD,$latM,($latm * 60),'000000000'),1,7)" />
								</xsl:if>
								<xsl:if test = "$toSeconds= 'false'">
									<xsl:value-of select="substring(concat($latD,$latM,$latm,'000000000'),1,7)" />
								</xsl:if>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:choose>
						...
						<xsl:choose>
							<xsl:when test = "$toDecimal = 'true'">
								...
								<xsl:value-of select="substring(concat($longFormat,'00000000'),1,8)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:if test = "$toSeconds= 'true'">
									<xsl:value-of select="substring(concat($longD,$longM,($longm*60),'0000000000'),1,8)" /> 
								</xsl:if> 
								<xsl:if test = "$toSeconds= 'false'">
									<xsl:value-of select="substring(concat($longD,$longM,$longm,'0000000000'),1,8)" /> 
								</xsl:if> 
							</xsl:otherwise>
						</xsl:choose>
