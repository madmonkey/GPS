					...
					<rawMessage>
						...
						<xsl:choose>
							<xsl:when test="$fromDecimal = 'true'">
								...
								<xsl:value-of select="substring(concat($latD,$latMS,$latSS,$latSR,'00000'),1,5)" /> 
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select ="substring(concat($latD,$latM,$latm,'00000'),1,5)"/>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:choose>
						...
						<xsl:value-of select="substring(concat($longD,$longM,$longm,'00000'),1,5)"/>
						<xsl:choose>
							<xsl:when test = "$fromDecimal = 'true'">
								...
								<xsl:value-of select="substring(concat($longD,$longMS,$longSS,$longSR,'00000'),1,5)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="substring(concat($longD,$longM,$longm,'00000'),1,5)"/>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:value-of select="string('FFFGGG2')"/>
					</rawMessage>
					<Type>0</Type>
				</xsl:when>
				<xsl:otherwise>
