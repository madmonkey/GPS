					...
					<xsl:when test ="string-length($latD)>0">
						...
						<rawMessage>
							...
							<xsl:when test = "$fromDecimal = 'true'">
								...
								<xsl:value-of select ="substring(concat($latD,$latMS,$latSS,$latSR,'000000000'),1,9)"/> 
							</xsl:when>
							<xsl:otherwise>
							...
							<xsl:when test = "$fromDecimal = 'true'">
								...
								<xsl:value-of select="substring(concat($longD,$longMS,$longSS,$longSR,'0000000000'),1,10)"/>
							</xsl:when>
							<xsl:otherwise>
