	...
	<!--Logon Status Validation.xsl (c) SunGard HTE Inc--> 
	<!--Logon Status Validation checks to see that the unit is currently logged-in and for valid LAT/LONG transaction (LN, PV) ensures that no other transaction goes to CAD-->
		...
		<xsl:template match="/">
			<GPSTransform>
				<xsl:choose>
					...
					<xsl:otherwise>
							<MessageStatus>3</MessageStatus>
							<rawMessage>UNIT IS NOT LOGGED INTO CAD</rawMessage>
							<Type>0</Type>
					</xsl:otherwise>
				</xsl:choose>
			</GPSTransform>
